...
def setup_menus():
    current_menu.submenu('.user.admin').register('admin.settings', 'Admin', order=1,
        visible_when=lambda: current_user.any_role('admin'))
    current_menu.submenu('.user.register').register('security.register', 'Register',
...
@bp.route('/profile')
@register_menu(bp, '.user.profile', 'Profile', visible_when=lambda: current_user.is_authenticated)
@login_required
def profile():
